    ...
    def get_optical_frame_coordinate(self):
        ...
        try:
            if abs(time_diff)<0.9:
                ...
                #print("SCALE FACTOR: ",scale_factor_row,", ",scale_factor_col)
                if len(self.boxes) > 0:
                    try:
                        for i in range(len(self.boxes)):
                            class_index = int(self.boxes[i][5])
                            #if str(self.class_names[class_index]) == "person":  # or str(self.class_names[class_index]) == "person":
                            ...
                            #nonzero_nonnan_data =  depths[(depths!= 0) & (~np.isnan(depths))]
...
def main():
        ...
        rospy.Subscriber("/camera/depth/camera_info", CameraInfo, tester.callback0, queue_size=100)
        rospy.Subscriber("/camera/color/image_raw", Image, tester.callback1, queue_size=1, buff_size=2_000_000)
        rospy.Subscriber("/camera/depth/image_rect_raw", Image, tester.callback2, queue_size=1, buff_size=2_000_000)
